<?php
	...
	//						echo $adminUsers->statut ; exit();	
				...
				include_once( _APP_PATH . 'views/admin_orders/display.php');
					...
					if(empty($ajaxItem['product'])) {
						$ajaxItem['product']['product_name'] = '';
						$ajaxItem['product']['product_type'] = '';
						$ajaxItem['product']['product_order_quantity'] = '';
					}
					...
					if(unlink(_WWW_PATH . $_POST['img_url'])) {
						$orders->order_id=$_POST["user_id"];
						if($orders->deleteAddressImg()){
							echo true;
						}
					}
					else {
				...
				if(isset($_POST) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
					...
					if($image_res){
						...
						if($this->normal_resize_image($image_res, $image_save_folder, $image_type, $max_image_size, $image_width, $image_height, $jpeg_quality))
						{
							...
							if ($orders->updateImage($image_url, $_GET["user"])) {
